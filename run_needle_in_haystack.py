...
class LLMNeedleHaystackTester:
    ...
    def __init__(self,
        ...
        if(self.model_provider in ["LLaMA3", "Mistral","LLaMA"]):
            ...
            if self.method in ['fullkv','H2O', 'chai' ,'snapkv', 'pyramidkv_generate', 'uncomp', "uncomp_stage"]:
            
                if self.model_provider == 'LLaMA3' or self.model_provider == 'LLaMA':
                    replace_llama(self.method,manager=self.manager)
                elif self.model_provider == 'Mistral':
                    replace_mistral(self.method,manager=self.manager)
                ...
                # llama3
                if self.method.lower() in ["uncomp","uncomp_stage"]:
                    window_sizes = 8
                else:
                    window_sizes = 8
                ...
                if method in self.manager.ahead_500:
                    self.manager.head_datas = torch.from_numpy(np.array(data_all_layers_1)).to(self.model_to_test.device).to(torch.bool)
                    self.manager.head_datas2 = torch.from_numpy(np.array(data_all_layers_2)).to(self.model_to_test.device)
    ...
    def evaluate_and_log(self, context_length, depth_percent):
        ...
        response = "the best thing to do in san francisco is " + response
        print("response is: ",response)
        print("self.needle is: ",self.needle)
        
        # assert 1==0
        test_end_time = time.time()
        test_elapsed_time = test_end_time - test_start_time
        if len(response) != 0:
            score = scorer.score(self.needle, response)['rouge1'].fmeasure*100
            manager.scorers.append(score)
        else:
            score = 0.0
        ...
        results = {
            ...
            'score' : score,
            'manager.scorers' : sum(manager.scorers) / len(manager.scorers),
        ...
        if self.print_ongoing_status:
            ...
            print (f"Score: {score}")
            print (f"self.manager.scorers: {sum(manager.scorers) / len(manager.scorers)}")
...
class Manager:
    ...
    hidden_delete_pyramidkv, hidden_delete_stage_and_ours, hidden_delete, hidden_entropy_test = [],[],[],[]
    chai_set, head_granularity, head0816, last_process, head_to_zero,chai = [],[],[],[],[],[]
    group_sampling, mask_implement, prefill_delete_head, draw_picture_set= [],[],[],[]
    search, hidden_entropy_set, hidden_delete, ahead_500_equal_code,ahead_500_gaussian_kernel = [],[],[],[],[]
    generate_stage,mask_implement,ahead_500_equal_code_reverse,extreme_compressibility_equal_code = [],[],[],[]
    delete_head_equal_code,delete_head_equal_code_2,time_survey_set = [],[],[]
    ahead_500_equal_code_group3,ahead_500_equal_code_group4,ahead_500_equal_code_group5 = [],[],[]
    ahead_500_equal_code_group8,ahead_500_equal_code_little_size_group8,ahead_500_group8 = [],[],[]
    ahead_500_equal_code_little_size = []
    ahead_500,ahead_10 = [],[]
    scorers = []
    not_update,mask_implement_padding = [],[]
    delet_head_set=[]
    rope,rope_position_ids_control,layer_differ_recent_n=[],[],[]
    pass
...
if __name__ == "__main__":
    ...
    parser.add_argument("--attn_implementation", type=str,  default="flash_attention_2", choices=["flash_attention_2", "sdpa", "eager","None"])
    ...
    parser.add_argument('--method', type=str, default="fullkv", choices=['fullkv','QS52', 'chai' ,'snapkv', 'pyramidkv_generate', 'uncomp','uncomp_stage'])
    parser.add_argument('--max_capacity_prompt', type=int, default=128)
    args = parser.parse_args()
    
    manager = Manager()
    manager.bsz = 1
    if args.model_provider in ["LLaMA"]:
    ...
    elif args.model_provider in ["LLaMA3"]:
        manager.chai_layers = manager.chai_layers_llama2
        manager.max_token = 4096
        manager.num_attention_heads = 32
        manager.num_hidden_layers = 32
    manager.method_name = args.method
    manager.ahead_500_equal_code.extend(["uncomp","uncomp_stage"])
    manager.head_granularity.extend(["uncomp","uncomp_stage"])
    manager.head0816.extend(["uncomp","uncomp_stage"])
    manager.ahead_500.extend(["uncomp","uncomp_stage"])
    manager.group_sampling.extend(["snapkv"])
    manager.group_sampling.extend(["pyramidkv_generate"])
    manager.chai.extend(["chai"])
    manager.hidden_delete_stage_and_ours.extend("uncomp_stage")
    manager.delet_head_set=[]
    
    if manager.method_name in manager.ahead_500:
        data_all_layers = []
        data_all_layers_1 = []
        data_all_layers_2 = []
        print("args.model_path is",args.model_name)
        for i in range(manager.num_hidden_layers):
            filename = "/home/avnet/mount_disk/sjh/UNComp/search/512/llama2-chat/query-all/head_type_search_layer" + str(i) + ".csv"
            data_layers = []
            ...
            print(top_half_indices)
            print(down_half_indices)
            new_data[top_half_indices] = 1
            data_all_layers.append([top_half_indices,down_half_indices])
            data_all_layers_1.append(new_data)
            data_all_layers_2.append(indices)
        manager.head_datas = torch.from_numpy(np.array(data_all_layers_1))
        manager.head_datas2 = torch.from_numpy(np.array(data_all_layers_2))
