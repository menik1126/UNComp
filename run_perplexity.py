...
logger = Logger(accelerator=accelerator,console_level=logging.DEBUG,log_file=None)
        ...
        for i in tqdm(range(0, len(split_data), args.eval_batch_size)):
            ...
            with torch.no_grad():
                outputs = model(
                    input_ids=batch_input_ids,
                    attention_mask=attention_mask,
                    labels=batch_input_ids  # 使用输入作为目标标签
                )
                loss = outputs.loss
                perplexity = torch.exp(loss)
                logger.info(f"Loss: {loss.item()}, Perplexity: {perplexity.item()}")
            
            from torch.nn import CrossEntropyLoss

            # 假设 input_ids 和 labels 是模型的输入和对应的目标输出
            outputs = model(
                input_ids=batch_input_ids,
                attention_mask=attention_mask,
                labels=batch_input_ids,  # 需要提供目标标签
                output_attentions=args.output_attentions
            )
...
class func_utils:
    ...
    def uncomp_sets(self,manager):
        ...
        if "uncomp_delele_head" in manager.method_name:
        ...
        lists_to_extend = [manager.delete_head_equal_code,
                           manager.extreme_compressibility_equal_code,
                           manager.ahead_500_equal_code,
                           ]
...
if __name__ == "__main__":
    ...
    if manager.method_name in manager.calib:
        datas = get_calib_data("wikitext2", tokenizer, args.model_path, nsamples = 100, seqlen=76000 )
